...
export default function EditedAddQuestion() {
  const [questions, setQuestions] = useState([]);
  const totalQuestions = 1
  const [questionNumber, setQuestionNumber] = useState(1);
  const { courseID, assignemntID } = useParams();
  const format = useLocation().state?.format;

  //states{courseID , assignemntID}
      ...
      if (selectedOption === "testcase") {
        if (
          ...
          setQuestionNumber(questionNumber + 1);
            ...
            const response = await http.put("/assignment/addQuestionInAssignment", {
              newQuestion,
              assignemntID,
            });
            console.log(
            ...
            if (response.data.success) {
              ...
              navigate(`/Teacher/ViewUploadedAssig/${courseID}/${assignemntID}`);
            }
  ...
  return (
    <Box sx={{ marginLeft: 2, marginRight: 2 }}>
      ...
      >
        <span className="underline">Add New Question</span>{" "}
      </p>
